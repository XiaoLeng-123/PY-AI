    ...
    try:
        ...
        if report_date:
            ...
            for report in financial_data:
                report_dt = report.get('REPORT_DATE', '')
                if report_dt:
                    # 简单字符串比较，寻找最接近的日期
                    diff = abs(len(report_dt) - len(report_date)) + sum(1 for a, b in zip(report_dt, report_date) if a != b)
                    if closest_diff is None or diff < closest_diff:
                        closest_diff = diff
                        target_report = report
            latest_report = target_report if target_report else financial_data[0]
        else:
            # 获取最新一期财报
        ...
        down_days = 0
        for i in range(1, len(prices)):
            if prices[i].close_price > prices[i-1].close_price:
                up_days += 1
            else:
                down_days += 1
        ...
        # 计算最大回撤
        peak = prices[0].close_price
        max_drawdown = 0
        for price in prices:
            if price.close_price > peak:
                peak = price.close_price
            drawdown = ((peak - price.close_price) / peak) * 100
            if drawdown > max_drawdown:
                max_drawdown = drawdown
        ...
        if len(returns) > 0:
            ...
            annualized_volatility = daily_volatility * (252 ** 0.5)  # 年化
        else:
        ...
        ma5 = prices_list[-5] if len(prices_list) >= 5 else prices_list[-1]
        ma10 = prices_list[-10] if len(prices_list) >= 10 else prices_list[-1]
        ma20 = prices_list[-20] if len(prices_list) >= 20 else prices_list[-1]
        ...
        # 支撑位和阻力位
        support = min(prices_list[-20:])  # 近20日最低价作为支撑
        resistance = max(prices_list[-20:])  # 近20日最高价作为阻力
        ...
        if len(prices_list) >= 5:
            return_5d = ((prices_list[-1] - prices_list[-5]) / prices_list[-5]) * 100
        else:
            return_5d = 0
            
        if len(prices_list) >= 10:
            return_10d = ((prices_list[-1] - prices_list[-10]) / prices_list[-10]) * 100
        else:
            return_10d = 0
